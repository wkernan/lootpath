...
Last updated: 2026-09-06 (QE-1, WKE-527: fork branch with the Upgrade Finder export and the Top Gear Download/Copy JSON controls; the live site had no Download JSON control at all - see §4, §5, §10, §11 and docs/qe-live-pr.md).
...
| `GetMapUIInfo().mapID` -> `GetInstanceForGameMap` (or the global `EJ_GetInstanceForMap`) works; what `SetPreviewMythicPlusLevel` does to loot rows; `EJ_LOOT_DATA_RECIEVED` timing; walk duration | M3-1 + M3-2 (`capture journal`, **in Restoration spec**) |
