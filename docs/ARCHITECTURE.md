...
- **Top Gear exports.** `src/General/Modules/TopGear/Report/TopGearJSONExport.ts`, present since at least 2026-05-20 (last touched by "handle duplicates"). Schema string `"qe-live-droptimizer"`, `version: 1`. Payload:
  ...
  **Refuted 2026-09-06 (WKE-527): there is no Download JSON control.** `downloadTopGearJSON` has no caller anywhere in `src`, in any of his 20 branches (`git grep` across `upstream/*`); the exporter has one commit and is dead code. The live site cannot produce the file today. The owner's fork branch `lootpath/upgrade-finder-export` wires it (Export menu: Download JSON, Copy JSON). Until that lands upstream, the owner gets an export by running the fork locally (`npm start` in `c:\Code\qe-live-fork`, import `/simc`, Top Gear, Export > Download JSON); the engine is the same code as the site.
...
| `Modules/Journal.lua` (adapter shipped M3-1 PR 1) | `ns.JournalAdapter` (every EJ call, raw, secret-guarded, plus the async walk and `capture journal`) + `ns.Journal` (pure aggregator, PR 2) | Every adapter entry point hands back `ns.Probe` packs unchanged (`{n=..., ...}`, `{absent=true}`, `{error=...}`) with each value through `ns.CopyRaw`; secrets are masked and counted in `Adapter.secretsSeen`. `Adapter.FUNCTION_NAMES` is the literal list of every client function the file calls, and `Availability()` reports which of them this client lacks. `Walk(opts, onDone)` selects instance / difficulty / loot filter / preview level per target, waits for `EJ_LOOT_DATA_RECIEVED` up to `LOOT_MAX_ATTEMPTS` x `LOOT_WAIT_SECONDS` (8 x 0.25s), counts events, waits, timeouts and `durationMs`, aborts on combat, and restores the view state. Aggregator contract unchanged: `{ [itemID] = { {instanceID, instanceName, encounterID, encounterName, difficultyID, itemLevel, slot, isRaid} } }`, cached in AceDB keyed `(build, seasonID, specID, difficultyID)`. **`itemLevel` is not a field of `EncounterJournalItemInfo`** (Blizzard docs, 2026-09-05): it comes from `C_Item.GetDetailedItemLevelInfo(link)` on the loot row's `link`, as the SimC addon does for vault links | aggregator yes; adapter only via captures (wowless lists every EJ function but its loot functions return nothing). `spec/journal_spec.lua` proves the behaviour *around* the client - probing, missing functions, secret masking, the async re-read, the bound, combat, restore - over a stub that is a placeholder for WKE-523's transcript |
...
- The M+ pool -> journal instance mapping is the step most likely to differ from any doc; the candidate path is in §9 and M3-2 proves it. Both lookups are recorded side by side, so the transcript decides rather than the code. Blizzard's own journal never calls `SetPreviewMythicPlusLevel`, so its effect on loot rows is unobserved.
...
- Every value in `spec/journal_spec.lua` is a stub placeholder. Those tests prove the adapter's behaviour around the client (probe, mask, wait, bound, abort, restore) and nothing whatever about the client's return shapes; that is WKE-523's job, and PR 2's aggregator waits for it.
